                ...
                <tr>
                    ...
                    <td class="text-center">
                        <c:if test="${book.bookStatus ==1}">
                            <button type="button" class="btn bg-olive btn-xs" onclick="returnBook(${book.bookId})">归还
                            </button>
                        </c:if>
                        <c:if test="${book.bookStatus ==2}">
                            ...
                            <c:if test="${USER_SESSION.userRole =='ADMIN'}">
                                <button type="button" class="btn bg-olive btn-xs" onclick="returnConfirm(${book.bookId})">
                                    归还确认
                                </button>
                            </c:if>
                        </c:if>
...
<script>
    ...
    bookVO.name = "${search.bookName}"
    bookVO.author = "${search.bookAuthor}"
    bookVO.press = "${search.bookPress}"
	/*分页效果*/
    pagination(pageargs);
</script>
</html>
